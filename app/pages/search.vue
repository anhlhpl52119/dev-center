...
const searchContentV2 = computed(() => {
  const rs: any[]
    = Object.values(data.value?.pages?.search?.results)?.flat() ?? [];
  return rs.map(i => ({
    title: i.title,
  ...
  <main>
    <div class="mt-80 text-center">
      ...
      <input
        ...
        type="text"
        class="border-abd-base bg-abg-base mt-24 w-full rounded-full border py-16 pr-72 pl-20 md:w-600"
        placeholder="검색어를 입력하세요."
        @keyup.enter="onSearch(searchKeyword)"
      >
    </div>

    <div class="mx-auto mt-40 grid max-w-1320 gap-16 px-8">
      <template v-for="(item, index) in searchContentV2" :key="index">
        <div
          class="bg-abg-base outline-abd-base bd-radius-32 hover:base-shadow p-30 transition hover:outline"
    ...
    <div class="mt-32 mb-66 flex py-8">
      <AppPagination
        v-if="totalPage >= 1"
